        ...
        while stack:
            ...
            if not self.is_leaf(parent):
                ...
                for child in children:
                    ...
                    if curr == '1' or self.id2tax[curr] == 'species':
                        continue

                    curr_pos = ordered_level_names_dict[self.id2tax[curr]]
                    child_pos = ordered_level_names_dict[self.id2tax[child]]

                    while curr_pos < (child_pos - 1):
                        # Remove current connetion
                        self.id2parent.pop(child)
                        self.id2children[curr].remove(child)

                        # Add new dummy node and connections
                        if f'-{curr}' not in self.id2children[curr]:
                            self.id2children[curr].append(f'-{curr}')
                        self.id2parent[f'-{curr}'] = curr

                        self.id2parent[child] = f'-{curr}'
                        try:
                            if child not in self.id2children[f'-{curr}']:
                                self.id2children[f'-{curr}'].append(child)
                        except KeyError:
                            self.id2children[f'-{curr}'] = [child]

                        self.id2tax[f'-{curr}'] = ordered_level_names[curr_pos
                                                                      + 1]
                        self.tax2ids[ordered_level_names[curr_pos +
                                                         1]].append(f'-{curr}')
