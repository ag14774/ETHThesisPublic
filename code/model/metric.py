...
class MoEMetricTransformer(BaseMetric):
    def __init__(self, metric):
        super(MoEMetricTransformer, self).__init__()
        self.metric = metric['constructor'](**metric['args'])

    def forward(self, output, target):
        o, _, _ = output
        return self.metric(o, target)

    def labels(self):
        return self.metric.labels()


class MoEExtraLoss(BaseMetric):
    def __init__(self):
        super(MoEExtraLoss, self).__init__()

    def forward(self, output, target):
        _, l_importance, l_load = output
